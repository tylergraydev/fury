...
#[tauri::command]
pub fn update_todo(state: State<'_, AppState>, request: UpdateTodoRequest) -> Result<(), AppError> {
    let db_lock = state.db.lock().unwrap();
    let db = db_lock
        .as_ref()
        .ok_or(AppError::DbError("DB not initialized".into()))?;
...
#[tauri::command]
pub fn delete_todo(state: State<'_, AppState>, todo_id: String) -> Result<(), AppError> {
    let id: Uuid = todo_id
        .parse()
...
#[tauri::command]
pub fn toggle_todo(state: State<'_, AppState>, todo_id: String) -> Result<bool, AppError> {
    let id: Uuid = todo_id
        .parse()
        .map_err(|_| AppError::DbError("Invalid todo ID".into()))?;
    let db_lock = state.db.lock().unwrap();
